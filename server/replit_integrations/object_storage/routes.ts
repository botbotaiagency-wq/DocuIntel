 ...
 * All routes require authentication (security retrofit: prevent unauthenticated upload/download).
 */
export function registerObjectStorageRoutes(app: Express, isAuthenticated: RequestHandler): void {
  ...
  if (process.env.USE_LOCAL_STORAGE === "1") {
    app.put("/api/local-upload/:id", isAuthenticated, expressRawBody(), (req: any, res) => {
    ...
    app.get("/api/local-object/uploads/:id", isAuthenticated, async (req, res) => {
      try {
  ...
  app.use("/objects", isAuthenticated, async (req, res) => {
    try {
