...
export function getSession() {
  ...
  if (dbUrl && process.env.NODE_ENV === "production") {
    ...
    return session({ ...baseOptions, store: sessionStore });
  }
...
export async function setupAuth(app: Express) {
  ...
  // Replit OIDC: only when REPL_ID is set (running on Replit)
  if (process.env.REPL_ID) {
    ...
    // Local dev: use /api/auth/logout from auth routes (session-based)
    app.get("/api/logout", (req, res) => {
      req.logout(() => {
        res.redirect("/");
      });
